...
// Sample currency data (matching HTML rows)
const initialCurrencies = [
  { id: 1, name: 'Dollars(Default)', symbol: '$' },
  { id: 2, name: 'Taka', symbol: '৳' },
  { id: 3, name: 'Rupee', symbol: '₹' },
  { id: 4, name: 'Rupee', symbol: '₹' },
  { id: 5, name: 'Rupee', symbol: '₹' },
  { id: 6, name: 'Rupee', symbol: '₹' },
  { id: 7, name: 'Rupee', symbol: '₹' },
  { id: 8, name: 'Rupee', symbol: '₹' },
  { id: 9, name: 'Rupee', symbol: '₹' },
  { id: 10, name: 'Rupee', symbol: '₹' },
  // extra for pagination
  { id: 11, name: 'Euro', symbol: '€' },
  { id: 12, name: 'Pound', symbol: '£' },
];
...
const Currencies = () => {
  ...
  const [editingCurrency, setEditingCurrency] = useState(null); // { id, name, symbol }

  // Delete modal state
  const [deleteModal, setDeleteModal] = useState({ open: false, currencyId: null, currencyName: '' });
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedIds(paginatedCurrencies.map((c) => c.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleAddSubmit = (e) => {
    ...
    setCurrencies([...currencies, { id: newId, name, symbol }]);
    closeAddDrawer();
  };
  ...
  const handleEditSubmit = (e) => {
    ...
    if (editingCurrency) {
      setCurrencies((prev) =>
        prev.map((c) =>
          c.id === editingCurrency.id ? { ...c, name, symbol } : c
        )
      );
    }
    closeEditDrawer();
  ...
  const openDeleteModal = (currency) => {
    setDeleteModal({ open: true, currencyId: currency.id, currencyName: currency.name });
  };

  const closeDeleteModal = () => {
    setDeleteModal({ open: false, currencyId: null, currencyName: '' });
  };

  const confirmDelete = () => {
    setCurrencies((prev) => prev.filter((c) => c.id !== deleteModal.currencyId));
    setSelectedIds((prev) => prev.filter((id) => id !== deleteModal.currencyId));
    closeDeleteModal();
  };
    ...
    <div className="dashboard-main-body">
                    ...
                    <th scope="col">
                      <div className="form-check style-check d-flex align-items-center">
                        <input
                          ...
                          type="checkbox"
                          checked={paginatedCurrencies.length > 0 && selectedIds.length === paginatedCurrencies.length}
                        ...
                        <td>
                          <div className="form-check style-check d-flex align-items-center">
                            <input
                              ...
                              type="checkbox"
                              checked={isSelected}
                              onChange={() => handleSelectRow(currency.id)}
                            />
        ...
        <form onSubmit={handleAddSubmit} className="d-flex flex-column p-20">
          <div className="row g-3">
            <div className="col-sm-12">
              <label htmlFor="currencyName" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">
                Currencies Name
              </label>
              <input
                type="text"
            ...
            <div className="col-sm-12">
              <label htmlFor="symbol" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">
                Symbol
              </label>
              <input
                type="text"
      ...
      >
        <form onSubmit={handleEditSubmit} className="d-flex flex-column p-20">
          <div className="row g-3">
            <div className="col-sm-12">
              <label htmlFor="currencyNameEdit" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">
                Currencies Name
              </label>
              <input
                type="text"
            ...
            <div className="col-sm-12">
              <label htmlFor="symbolEdit" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">
                Symbol
              </label>
              <input
                type="text"
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete "${deleteModal.currencyName}"?`}
