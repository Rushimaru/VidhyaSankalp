...
// Sample language data (matching HTML rows)
...
const Languages = () => {
  ...
  const [editingLanguage, setEditingLanguage] = useState(null); // { id, name }

  // Delete modal state
  const [deleteModal, setDeleteModal] = useState({ open: false, languageId: null, languageName: '' });
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedIds(paginatedLanguages.map((l) => l.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleAddSubmit = (e) => {
    ...
    setLanguages([...languages, { id: newId, name }]);
    closeAddDrawer();
  };
  ...
  const handleEditSubmit = (e) => {
    ...
    if (editingLanguage) {
      setLanguages((prev) =>
        prev.map((l) =>
          l.id === editingLanguage.id ? { ...l, name } : l
        )
      );
    }
    closeEditDrawer();
  ...
  const openDeleteModal = (language) => {
    setDeleteModal({ open: true, languageId: language.id, languageName: language.name });
  };

  const closeDeleteModal = () => {
    setDeleteModal({ open: false, languageId: null, languageName: '' });
  };

  const confirmDelete = () => {
    setLanguages((prev) => prev.filter((l) => l.id !== deleteModal.languageId));
    setSelectedIds((prev) => prev.filter((id) => id !== deleteModal.languageId));
    closeDeleteModal();
  };
    ...
    <div className="dashboard-main-body">
                    ...
                    <th scope="col">
                      <div className="form-check style-check d-flex align-items-center">
                        <input
                          ...
                          type="checkbox"
                          checked={paginatedLanguages.length > 0 && selectedIds.length === paginatedLanguages.length}
                        ...
                        <td>
                          <div className="form-check style-check d-flex align-items-center">
                            <input
                              ...
                              type="checkbox"
                              checked={isSelected}
                              onChange={() => handleSelectRow(lang.id)}
                            />
        ...
        <form onSubmit={handleAddSubmit} className="d-flex flex-column p-20">
          <div className="row g-3">
            <div className="col-sm-12">
              <label htmlFor="languageName" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">
                Language Name
              </label>
              <input
                type="text"
      ...
      >
        <form onSubmit={handleEditSubmit} className="d-flex flex-column p-20">
          <div className="row g-3">
            <div className="col-sm-12">
              <label htmlFor="languageNameEdit" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">
                Language Name
              </label>
              <input
                type="text"
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete "${deleteModal.languageName}"?`}
