...
const EditGuardian = () => {
  ...
  const guardianData = location.state?.guardian || null;
  ...
  // Load guardian data if available (for editing)
  useEffect(() => {
    if (guardianData) {
      setFormData((prev) => ({
        ...prev,
        ...
        phoneNumber: guardianData.phone || '',
        occupation: guardianData.occupation || '',
        guardianAddress: guardianData.guardianAddress || '',
        email: guardianData.email || '',
        password: guardianData.password || '',
        // Note: guardianPhoto is a file, not stored in state like this; you'd handle separately
      }));
    }
  }, [guardianData]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('Updated Guardian Data:', formData);
    // Here you would send updated data to API
    alert('Guardian updated successfully!');
    navigate('/guardians'); // Redirect to guardian list after save
  };

  // Handle reset (revert to original guardian data)
  const handleReset = () => {
    if (guardianData) {
      setFormData({
        guardianType: guardianData.guardianType || '',
      ...
      // If no original data, clear form (should not happen in edit)
      setFormData({
        guardianType: '',
        guardianName: '',
        phoneNumber: '',
      ...
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          ...
          <div>
            ...
            <Link to="/guardians" className="text-secondary-light hover-text-primary hover-underline">
              / Guardian
            </Link>
                  ...
                  <div className="col-xl-4 col-sm-6">
                    <div>
                      ...
                      {formData.guardianPhoto && (
                        <p className="mt-2 text-sm text-success-600">File selected: {formData.guardianPhoto.name}</p>
                      )}
                    </div>
                  </div>
                </div>
